    ...
    <div class="col-md-12">
        <div class="box box-info">
            <div class="box-header with-border">
                <h3 class="box-title"><i class="fa fa-file-text-o mr-2"></i> Form Input Gaji Sablon Borongan Dalam</h3>
            </div>
            <form action="<?php echo $action ?>" method="POST">
                <div class="box-body">
                    ...
                    <div class="row" style="margin-top: 20px;">
                        <div class="col-md-12">
                            <table class="table table-bordered table-striped" id="item_table_dalam">
                                <thead>
                                    <tr class="bg-info">
                                        <th width="30%">Kode PO (Dalam)</th>
                                        <th>Gambar</th>
                                        <th>Model</th>
                                        <th width="10%">Lusin</th>
                                        <th width="10%">Putaran</th>
                                        <th width="15%">Harga</th>
                                        <th width="5%" class="text-center"><button type="button" name="add" class="btn btn-success btn-sm add_dalam"><i class="fa fa-plus"></i></button></th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
...
$(document).ready(function(){
    var i=0;
    $(document).on('click', '.add_dalam', function(){
        var html = '';
        html += '<tr>';
        html += '<td>';
        ...
        html += '<option value="<?php echo $value['id_produksi_po'] ?>"><?php echo addslashes($value['kode_po']) ?></option>';
        <?php endforeach ?>
        html += '</select>';
        html += '</td>';
        ...
        html += '<td><input type="number" step="any" name="prods['+i+'][lusin]" class="form-control" required></td>';
        html += '<td><input type="number" step="any" class="form-control" name="prods['+i+'][putaran]"></td>';
        html += '<td><input type="number" step="any" class="form-control" name="prods['+i+'][harga]" required></td>';
        ...
        i++;
        $('#item_table_dalam tbody').append(html);
