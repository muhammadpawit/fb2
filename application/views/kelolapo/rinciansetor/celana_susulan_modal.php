<div class="row">
    <div class="col-md-12">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label>Tanggal</label>
                    <input type="date" class="form-control" id="tanggal_susulan" name="tanggal" value="<?php echo date('Y-m-d') ?>" required>
                </div>
            </div>
        </div>
    </div>
    ...
    <form action="<?php echo $editaction ?>" method="post">
        ...
        <input type="hidden" name="tanggal" id="tanggal_hidden_susulan" value="<?php echo date('Y-m-d') ?>">
        <input type="hidden" class="form-control" name="tgl" value="<?php echo $poProd['created_date'] ?>" >
        <div class="table-responsive">
            <table class="table table-bordered" id="item_table_susulan">
                <thead>
                    <tr>
                        <th>SIZE</th>
                        <th>DZ</th>
                        ...
                        <th>CLM</th>
                        <th>KET</th>
                        <th><button type="button" class="btn btn-success btn-sm add-row-susulan"><i class="fa fa-plus"></i></button></th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($setorcmtjahititem)): ?>
                        ...
                        <tr>
                            ...
                            <td><button type="button" class="btn btn-danger btn-sm remove-row-susulan"><i class="fa fa-trash"></i></button></td>
                        </tr>
                        <?php endforeach ?>
                    <?php endif ?>
                </tbody>
...
<script>
    $(document).ready(function() {
        $('#tanggal_susulan').on('change', function() {
            $('#tanggal_hidden_susulan').val($(this).val());
        });

        $('.add-row-susulan').off('click').on('click', function() {
            var html = '';
            html += '<tr>';
            html += '<td>';
            ...
            html += '<td><button type="button" class="btn btn-danger btn-sm remove-row-susulan"><i class="fa fa-trash"></i></button></td></tr>';
            $('#item_table_susulan tbody').append(html);
        });

        $(document).on('click', '.remove-row-susulan', function() {
