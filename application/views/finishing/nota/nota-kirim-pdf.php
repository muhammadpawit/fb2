<html>
  ...
  <tbody>
    ...
    <?php foreach($prods as $p) { ?>
        ...
        <?php foreach($sizes as $size) { 
            ...
            foreach($p['details'] as $d) {
                if($d['rincian_size'] == $size) { 
                    $lusinValue = $d['rincian_lusin'];
                    $pcsValue = $d['rincian_piece'];
                    break;
                }
            } 
        ?>
      ...
      <?php 
        $no++; 
        $totalQty += $p['jumlah_piece_diterima']; 
      ?>
    <?php } ?>
  </tbody>
  <tfoot>
    <tr>
      <td align="center" colspan="<?php echo (count($sizes) * 2)+2 ?>"><b>Grand Total Qty</b></td>
      <td align="center"><strong><?php echo number_format($totalQty) ?></strong></td>
      <td></td>
    </tr>
  </tfoot>
</table>
