...
$(document).ready(function(){
    ...
    $(document).on('click', '.addborongan', function(){
        var html = '';
        html += '<tr>';
        html += '<td width="300"><select type="text" class="form-control selectpicker kodepo" name="products['+i+'][kode_po]" data-size="4" data-live-search="true" data-title="Pilih item" required><option value="">Pilih PO</option><?php foreach ($kodepo as $key => $po) { ?><option value="<?php echo $po['kode_po'] ?>" data-item="<?php echo $po['kode_po'] ?>"><?php echo $po['nama_po'].' '.$po['kode_po'] ?></option><?php } ?></select></td>';
    ...
    $(document).on('change', '.selectpicker', function(e){
        ...
        $.get( "<?php echo BASEURL.'finishing/biaya_finishing_cucian' ?>", { kodepo: dataItem } )
          .done(function( data ) {
