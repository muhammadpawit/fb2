<form class="form-group" action="<?php echo $action?>" method="post">
                    <table class="table">
                      ...
                      <tr>
                        ...
                        <td>:</td>
                        <td>
                          <select class="form-control select2bs4" name="cmtName" required title="Pilih Nama CMT" data-live-search="true" required>
                            <?php foreach($listcmt as $c){?>
                            <option value="<?php echo $c['id_cmt']?>-<?php echo $c['cmt_name']?>">
                              <?php echo strtolower($c['cmt_name'])?>
                            </option>
                            <?php } ?>
                          </select>
                        </td>
...
<script type="text/javascript">
    ...
    function addkirimgudang(){
        var html = '';
        html += '<tr>';
        html += '<td><select type="text" class="form-control selectpicker kodepo" name="products['+i+'][kode_po]" data-size="4" data-live-search="true" data-title="Pilih item" required><?php foreach ($kodepo as $key => $po) { ?><option value="<?php echo $po['id'] ?>" data-item="<?php echo $po['id'] ?>"><?php echo $po['nama'] ?></option><?php } ?></select></td>';
        ...
        html += '<td><input type="number" class="form-control" name="products['+i+'][rincian_po]"  required ></td>';
