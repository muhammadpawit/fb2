...
class Json extends CI_Controller {
	...
	public function monitor(){
		...
		$nomors=$start+1;
		$allpo=[];
		$sql="SELECT * FROM produksi_po LEFT JOIN master_jenis_po ON (master_jenis_po.nama_jenis_po=produksi_po.nama_po) WHERE produksi_po.hapus=0 AND kode_po IN(SELECT kode_po FROM konveksi_buku_potongan where hapus=0 ) AND nama_po NOT IN('BJF','BJK','BJH') ";
		if($jenispo!='null'){
	...
	{
		$post = $this->input->get();
		$data = $this->GlobalModel->QueryManualRow("SELECT SUM(kbp.qty_tot_pcs) as kirimpcs, kbp.cmt_job_price as harga FROM kelolapo_kirim_setor kbp WHERE idpo='".$post['kodepo']."' AND hapus=0 AND kategori_cmt='JAHIT' AND progress='KIRIM' AND id_master_cmt='".$post['cmt']."' ");
		echo json_encode($data);
	}
	...
	public function cekpotonganalat(){
		...
		$cek=$this->GlobalModel->QueryManual("SELECT a.*, b.nama_item, b.harga_item FROM distribusi_alat_sukabumi a
		 ...
		 WHERE idcmt='".$data['cmt']."' AND validasi=1 AND a.hapus=0 AND idpembayaran IS NULL ");
		if(!empty($cek)){
			$hasil=$cek;
		}
		echo json_encode($hasil);
