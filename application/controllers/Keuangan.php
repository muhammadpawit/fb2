...
class Keuangan extends CI_Controller {
	...
	public function pinjamankaryawan(){
		...
		if(!empty($karyawan)){
			...
			$sql.=" ORDER BY status ASC, tanggal DESC";
	...
	public function piutang()
	{
		...
		foreach($resutls as $result){
			...
			$supplier=$this->GlobalModel->getDataRow('master_supplier',array('id'=>$result['supplier']));
			$products=$this->GlobalModel->getData('penerimaan_item_detail',array('hapus'=>0,'validasi'=>1,'penerimaan_item_id'=>$result['id']));
			$data['items'][]=array(
				'id'=>$result['id'],
				'tanggal'=>date('d-m-Y',strtotime($result['tanggal'])),
				'nosj'=>$result['nosj'],
	...
	public function getmutasi() {
        ...
        if (!empty($data)) {
			...
			echo '<select name="cat" id="cat" class="form-control select2bs4">';
			echo '<option value="*"' . ($data['bagian'] == '*' ? ' selected' : '') . '>Semua</option>';
			echo '<option value="0"' . ($data['bagian'] == '0' ? ' selected' : '') . '>Default</option>';
			echo '<option value="1"' . ($data['bagian'] == '1' ? ' selected' : '') . '>Konveksi</option>';
			echo '<option value="2"' . ($data['bagian'] == '2' ? ' selected' : '') . '>Bordir</option>';
