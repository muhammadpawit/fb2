...
class Pembayaran extends CI_Controller {
	...
	public function timpotongdetail($id){
		...
		$data['claims']=$this->GlobalModel->getDataRow('claim_timpotong',array('id_pembayaran'=>$id,'hapus'=>0));
	...
	public function cmtjahitdetail($id){
		...
		foreach($products as $p){
			...
			if(!empty($pot)){
				$pots= $pot['hasil_pieces_potongan'] > 0 ? $pot['hasil_pieces_potongan'] : ($pot['dz_variasi']*12);
			}else{
		...
		if(isset($get['excel'])){
			...
			$paper = array(0, 0, 800, 1250);  // Ukuran kertas kustom (sesuaikan jika perlu)
