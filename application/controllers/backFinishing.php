...
class Finishing extends CI_Controller {
	...
	public function hppproduksidetail($kodepo='')
	{
		...
		// $baru=0;
		// foreach ($viewData['bahan'] as $key => $bahanAja){
		// 	if ($bahanAja['bahan_kategori'] == "UTAMA"){
		// 		$baru+=($bahanAja['harga_item'] * $viewData['bukupotongan']['jumlah_pemakaian_bahan_utama']);
		// 	}else if($bahanAja['bahan_kategori'] == "CELANA"){
		// 		$baru+=($bahanAja['harga_item'] * $viewData['bukupotongan']['jumlah_pemakaian_bahan_variasi']);
		// 	} else if($bahanAja['bahan_kategori'] == "KAINKANTONG") {
		// 		$baru+=($viewData['bahanKantong']['harga_item'] * $viewData['bahanKantong']['ukuran_item_keluar']);
		// 	}
		// }
		// $baru+=($viewData['master_harga_potongan']['harga_potongan']*12);
		// foreach ($viewData['cmt'] as $key => $cmtt){
		// 	if($cmtt['kategori_cmt']=='JAHIT'){
		// 		if (isset($viewData['bordirer'])){ 
		// 			if ($cmtt['kategori_cmt'] == "BORDIR"){
