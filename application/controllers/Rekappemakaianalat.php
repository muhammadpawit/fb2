...
class Rekappemakaianalat extends CI_Controller {
...

	{
		...
		$sql="SELECT * FROM gudang_persediaan_item WHERE hapus=0 AND id_persediaan IN(SELECT id_persediaan FROM gudang_item_keluar WHERE hapus=0) ";
		...
		$data['excel']=BASEURL.'Rekappemakaianalat?&excel=2'.$url;
