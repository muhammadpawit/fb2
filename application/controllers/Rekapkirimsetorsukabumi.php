...
class Rekapkirimsetorsukabumi extends CI_Controller {
	...
	public function index(){
		...
		$data['title']='Laporan Rekap Kirim Setor Sukabumi ';
		...
		$data['excel']=BASEURL.'Rekapkirimsetorsukabumi?&excel=true'.$url;
		...
		$data['datacmt']=$this->GlobalModel->QueryManual("SELECT * FROM master_cmt WHERE hapus=0 AND lokasi IN(3)");

		$sql = "SELECT * FROM master_cmt WHERE lokasi IN(3) AND hapus=0";
		...
		$datacmt=$this->GlobalModel->QueryManual($sql);

		$prods=[];

		foreach($datacmt as  $r){

			$prods=$this->ReportModel->rekap_cmt($r['id_cmt'],'KIRIM',$bulan,$tahun);
			...
			/*foreach(nama_po() as $p){
				...
				$jml=$this->KirimsetorModel->rekapjumlah($p['id_jenis_po'],$r['id_cmt'],'KIRIM',$bulan,$tahun);
				...
				$data['products'][]=array(
					...
					'kirimdz'=>($this->KirimsetorModel->rekappcs($p['id_jenis_po'],$r['id_cmt'],'KIRIM',$bulan,$tahun))/12,

					'kirimpcs'=>($this->KirimsetorModel->rekappcs($p['id_jenis_po'],$r['id_cmt'],'KIRIM',$bulan,$tahun)),

					'jmlsetor'=>$this->KirimsetorModel->rekapjumlah($p['id_jenis_po'],$r['id_cmt'],'SETOR',$bulan,$tahun),

					'setordz'=>($this->KirimsetorModel->rekappcs($p['id_jenis_po'],$r['id_cmt'],'SETOR',$bulan,$tahun))/12,

					'setorpcs'=>($this->KirimsetorModel->rekappcs($p['id_jenis_po'],$r['id_cmt'],'SETOR',$bulan,$tahun)),
