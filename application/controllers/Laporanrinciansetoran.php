...
class Laporanrinciansetoran extends CI_Controller {
	...
	public function index(){
		...
		foreach($res as $r){
			...
			$pcs_kirim=$this->GlobalModel->QueryManualRow("SELECT SUM(qty_tot_pcs) as total FROM kelolapo_kirim_setor WHERE idpo='".$r['idpo']."' AND lower(nama_cmt)='".strtolower($r['nama_cmt'])."' AND hapus=0 AND kategori_cmt='JAHIT' AND progress='KIRIM' ");
			$data['prods'][]=array(
				'no'=>$no,
	...
	public function celana(){
		...
		foreach($res as $r){
			...
			$data['prods'][]=array(
				...
				'pcs_setor'=>$r['jml_setor_qty'],
				'pcs_bagus'=>($r['jumlah_piece_diterima']-$r['bangke_qty']-$r['barang_cacad_qty']),
				'size'=>$this->GlobalModel->GetData('kelolapo_rincian_setor_cmt_finish_celana',array('kode_po'=>$r['kode_po'])),
				'bangke'=>$r['bangke_qty'],
				'bs'=>$r['barang_cacad_qty'],
				'cabang'=>!empty($lokasi)?$lokasi['lokasi']:'',
				'cmt'=>$r['nama_cmt'],
