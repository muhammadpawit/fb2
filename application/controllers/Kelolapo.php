...
class Kelolapo extends CI_Controller
{
  ...
  Your browser does not support the audio element.
	...
	public function posave()
	{
		...
		$cekart = $this->GlobalModel->GetData('produksi_po', array('hapus' => 0, 'kode_artikel' => $post['artikel']));
	...
	public function searchPO()
	{
		...
		// $bahans=$this->GlobalModel->QueryManual("SELECT * FROM gudang_bahan_keluar WHERE hapus=0 AND idpo='$kode_po' AND bahan_kategori LIKE '%UTAMA%' AND nama_item_keluar NOT IN (SELECT kode_bahan_potongan FROM konveksi_buku_potongan_utama WHERE hapus=0 AND idpo='$kode_po' ) AND warna_item_keluar NOT IN (SELECT warna_potongan FROM konveksi_buku_potongan_utama WHERE hapus=0 AND idpo='$kode_po' ) ");
		$bahans = $this->GlobalModel->QueryManual("SELECT * FROM gudang_bahan_keluar WHERE hapus=0 AND idpo='$kode_po' AND bahan_kategori LIKE '%UTAMA%' AND hapus=0 ");
		foreach ($bahans as $b) {
			echo "<tr>";
	...
	public function searchPObahan()
	{
		...
		// $bahans=$this->GlobalModel->QueryManual("SELECT * FROM gudang_bahan_keluar WHERE hapus=0 AND idpo='$kode_po' AND bahan_kategori LIKE '%CELANA%' AND nama_item_keluar NOT IN (SELECT kode_bahan_potongan FROM konveksi_buku_potongan_variasi WHERE hapus=0 AND idpo='$kode_po' ) AND warna_item_keluar NOT IN (SELECT warna_potongan FROM konveksi_buku_potongan_variasi WHERE hapus=0 AND idpo='$kode_po' ) ");
		$bahans = $this->GlobalModel->QueryManual("SELECT * FROM gudang_bahan_keluar WHERE hapus=0 AND idpo='$kode_po' AND bahan_kategori LIKE '%CELANA%' ");
		foreach ($bahans as $b) {
			echo "<tr>";
	...
	public function kirimcmtsabloncetak($id = '', $type = '')
	{
		...
		$data['nota'] = 'Sablon';
		$data['kirim'] = $this->GlobalModel->getDataRow('kirimcmtsablon', array('id' => $id));
		$data['kirims'] = $this->GlobalModel->getData('kirimcmtsablon_detail', array('hapus' => 0, 'idkirim' => $id));
		$data['cmt'] = $this->GlobalModel->getDataRow('master_cmt', array('id_cmt' => $data['kirim']['idcmt']));
		if ($type == 2) {
			$pdf = false;
		} else {
	...
	public function kirimcmtsablonedit($id = '', $kodepo = '')
	{
		...
		foreach ($kirims as $k) {
			...
			$po = $this->GlobalModel->getDataRow('produksi_po', array('id_produksi_po' => $k['idpo']));
			$data['kirims'][] = array(
				'kode_po' => isset($po['kode_po']) ? $po['kode_po'] : $k['idpo'],
				'idpo' => $k['idpo'],
				'rincian_po' => $k['rincian_po'],
				'job' => $job['id'],
				'jumlah_pcs' => $k['jumlah_pcs'],
	...
	public function pengirimancmt()
	{
		...
		$data['listpo'] = $this->GlobalModel->queryManual('SELECT id_produksi_po, kode_po FROM produksi_po WHERE hapus=0 ORDER BY id_produksi_po DESC');
		$filter = array(
			'hapus' => 0,
		);
		$results = array();
		...
		if (!empty($kodepo) && $kodepo != '*') {
			$po_info = $this->GlobalModel->getDataRow('produksi_po', array('id_produksi_po' => $kodepo));
			if (!$po_info) {
				$po_info = $this->GlobalModel->getDataRow('produksi_po', array('kode_po' => $kodepo));
			}
			if ($po_info) {
				$id_po = $po_info['id_produksi_po'];
				$code_po = $po_info['kode_po'];
				$sql .= " AND id IN (SELECT idkirim FROM kirimcmt_detail WHERE (kode_po='$id_po' OR kode_po='$code_po') AND hapus=0) ";
			} else {
				$sql .= " AND id IN (SELECT idkirim FROM kirimcmt_detail WHERE kode_po='$kodepo' AND hapus=0) ";
			}
		}

		if ((empty($cmt) || $cmt == '*') && (empty($sj) || $sj == '*') && (empty($kodepo) || $kodepo == '*')) {
			if (!empty($tanggal1)) {
				$sql .= " AND date(tanggal) BETWEEN '" . $tanggal1 . "' AND '" . $tanggal2 . "' ";
			}
		}
		...
		foreach ($results as $result) {
			...
			$namacmt = $this->GlobalModel->getDataRow('master_cmt', array('id_cmt' => $result['idcmt']));
			if (!empty($kodepo) && $kodepo != '*') {
				if (isset($po_info) && $po_info) {
					$id_po = $po_info['id_produksi_po'];
					$code_po = $po_info['kode_po'];
					$dets = $this->GlobalModel->QueryManual("SELECT * FROM kirimcmt_detail WHERE hapus=0 AND idkirim='" . $result['id'] . "' AND (kode_po='$id_po' OR kode_po='$code_po')");
				} else {
					$dets = $this->GlobalModel->QueryManual("SELECT * FROM kirimcmt_detail WHERE hapus=0 AND idkirim='" . $result['id'] . "' AND kode_po='$kodepo'");
				}
			} else {
	...
	public function pengirimancmt_pdf()
	{
		...
		$cmt = isset($get['cmt']) ? $get['cmt'] : null;
		$sj = isset($get['sj']) ? $get['sj'] : null;
		$kodepo = isset($get['kodepo']) ? $get['kodepo'] : null;
	...
	public function kirimcmtsave()
	{
		...
		if (isset($post['tanggal'])) {
			...
			foreach ($post['products'] as $p) {
				...
				$id_produksi_po = !empty($masterpo['id_produksi_po']) ? $masterpo['id_produksi_po'] : (is_numeric($p['kode_po']) ? $p['kode_po'] : 0);
				$kode_po_str    = !empty($masterpo['kode_po']) ? $masterpo['kode_po'] : $p['kode_po'];

				$totalkirim += ($p['jumlah_pcs']);
				$detail = array(
					'idkirim' => $id,
					'kode_po' => $id_produksi_po,
					'cmtjob' => $p['cmtjob'],
					'rincian_po' => $p['rincian_po'],
					'jumlah_pcs' => $p['jumlah_pcs'],
					'keterangan' => $p['keterangan'],
	...
	public function kirimcmteditsave()
	{
		...
		$totalkirim = 0;
		foreach ($post['prods'] as $p) {
			$cek_diklo = $this->GlobalModel->getDataRow('kelolapo_kirim_setor', array('hapus' => 0, 'kode_nota_cmt' => $post['kode_nota'], 'kategori_cmt' => 'JAHIT', 'id_master_cmt' => $post['idcmt']));
			$totalkirim += ($p['jumlah_pcs']);
			$rp = explode('-', $p['job']);
			if (empty($cek_diklo)) {
				...
				$kodepo_val = !empty($masterpo['kode_po']) ? $masterpo['kode_po'] : $p['kode_po'];
				$idpo_val = !empty($masterpo['id_produksi_po']) ? $masterpo['id_produksi_po'] : (is_numeric($p['kode_po']) ? $p['kode_po'] : 0);
  ...
  Your browser does not support the audio element.
	...
	public function sync_missing_kirimcmt()
	{
		$sql = "SELECT k.id as idkirim, k.tanggal, k.idcmt, kd.kode_po as detail_po, kd.cmtjob, kd.jumlah_pcs, kd.jml_barang
				FROM kirimcmt k 
				JOIN kirimcmt_detail kd ON k.id = kd.idkirim 
				WHERE k.hapus = 0 AND kd.hapus = 0";
		...
		if ($isCli) {
			echo "Memulai proses sinkronisasi massal data Jahit ($total data)... \n";
		}
		...
		if (!empty($details)) {
			foreach ($details as $d) {
				...
				if (empty($cek) && !empty($kode_po_str)) {
					$jobprice = $this->GlobalModel->getDataRow('master_job', array('id' => $d['cmtjob']));
