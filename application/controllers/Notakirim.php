...
class Notakirim extends CI_Controller
{
	...
	public function next_save()
	{
		...
		$cek_diterima_reguler = $this->GlobalModel->QueryManualRow("SELECT COALESCE(SUM(jml_setor_qty+pcs_setor_qty-bangke_qty-barang_cacad_qty),0) as total FROM kelolapo_rincian_setor_cmt WHERE kode_po='" . $idpo['kode_po'] . "' ");
		$cek_diterima_celana = $this->GlobalModel->QueryManualRow("SELECT COALESCE(SUM(jml_setor_qty+pcs_setor_qty-bangke_qty-barang_cacad_qty),0) as total FROM kelolapo_rincian_setor_cmt_celana WHERE kode_po LIKE '%" . $idpo['kode_po'] . "%' OR idpo='" . $idpo['id_produksi_po'] . "' ");

		$total_diterima = (float)($cek_diterima_reguler['total'] ?? 0) + (float)($cek_diterima_celana['total'] ?? 0);

		if ($dikirim > $total_diterima) {
