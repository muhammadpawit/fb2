...
class Transport extends CI_Controller {
	...
	function __construct() {
		...
		$this->page='newtheme/page/transport/';
		$this->url=BASEURL.'Transport/';
		$this->load->model('TransportModel');
	}

	public function pendapatan(){
		$data=[];
		$data['title']='Pendapatan transport homie noya ';
		$no=1;
		$get=$this->input->get();
		$url='';
		...
		//pre($filter);
		$products=$this->TransportModel->getdata($filter);
		$data['products'] = $products;
		$data['tambah']=$this->url.'tambah';
		$data['page']=$this->page.'pendapatan';
		$data['url']=$this->url;
