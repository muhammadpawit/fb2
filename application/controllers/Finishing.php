...
class Finishing extends CI_Controller {
	...
	public function kirimgudang($kodepo='')
	{
		...
		$viewData['rincian'] = $this->GlobalModel->queryManual('SELECT * FROM produksi_po pp JOIN kelolapo_kirim_setor kks ON pp.kode_po=kks.kode_po WHERE kks.progress="'.'SELESAI'.'" OR kks.progress="'.'FINISHING'.'" AND pp.id_produksi_po NOT IN(SELECT idpo FROM finishing_kirim_gudang) GROUP BY kks.idpo ORDER BY kks.kode_po ASC');
