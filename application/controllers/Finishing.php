...
class Finishing extends CI_Controller {
	...
	public function gajifinishing_add(){
		...
		$results =$this->GlobalModel->QueryManual("SELECT * FROM karyawan_harian WHERE hapus=0 and tipe=1 AND status_gaji=1 AND (bagian LIKE '%FINISHING%' OR lower(bagian) LIKE '%bpo%') ");
	...
	public function hppdetail($kodepo){
		...
		$viewData['jahit']	= $this->GlobalModel->QueryManualRow("SELECT * FROM kelolapo_kirim_setor WHERE kategori_cmt='JAHIT' AND hapus=0 AND idpo='".$po['id_produksi_po']."' ");
	...
	public function kirimgudang($kodepo='')
	{
		...
		$viewData['rincian'] = $this->GlobalModel->queryManual("SELECT * FROM produksi_po pp JOIN kelolapo_kirim_setor kks ON pp.id_produksi_po=kks.idpo WHERE kks.kategori_cmt='JAHIT' AND pp.id_produksi_po NOT IN(SELECT idpo FROM finishing_kirim_gudang) GROUP BY kks.idpo ORDER BY kks.kode_po ASC");
