...
class KasbonModel extends CI_Model
{
    ...
    public function kasbon($bulan, $tahun, $idkaryawan)
    {
        ...
        if (!empty($data)) {
            foreach ($data as $d) {
                $s = " SELECT nominal_acc as nominal,tanggal FROM kasbon WHERE DATE(tanggal)='" . $d['tanggal'] . "' AND hapus=0 AND idkaryawan='" . $idkaryawan . "' ";
    ...
    {
        $hasil = 0;
        $sql = "SELECT nominal_acc as nominal FROM kasbon WHERE hapus=0 ";
    ...
    {
        $hasil = 0;
        $sql = "SELECT COALESCE(SUM(nominal_acc),0) as nominal FROM kasbon WHERE hapus=0 ";
        // $sql.=" AND MONTH(tanggal)='".$bulan."' ";
        // $sql.=" AND YEAR(tanggal)='".$tahun."' ";
        $sql .= " AND DATE(tanggal) BETWEEN '" . $bulan . "' ";
        $sql .= " AND '" . $tahun . "' ";
