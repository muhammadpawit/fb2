...
class GlobalModel extends CI_Model {
	...
	public function getStokPOs($idcmt,$idjenis){
		...
		$bangke="SELECT COALESCE(SUM(bangke_qty),0) as total FROM kelolapo_rincian_setor_cmt rpo ";
			$bangke.=" LEFT JOIN kelolapo_kirim_setor kbp ON kbp.kode_po=rpo.kode_po LEFT JOIN produksi_po p ON(p.id_produksi_po=kbp.idpo) LEFT JOIN master_jenis_po mjp ON(mjp.nama_jenis_po=p.nama_po) WHERE kbp.id_master_cmt='$idcmt' and  mjp.tampil=1 AND kbp.kategori_cmt='JAHIT' AND kbp.progress='SETOR' AND kbp.hapus=0 AND mjp.id_jenis_po='$idjenis' ";
			if(!empty($bulan)){
		...
		$query="SELECT count(pp.kode_po) as jmlpo,SUM(kd.qty_tot_pcs) as pcs FROM kelolapo_kirim_setor kd LEFT JOIN produksi_po pp ON(kd.idpo=pp.id_produksi_po) LEFT JOIN master_jenis_po mjp ON(mjp.nama_jenis_po=pp.nama_po) WHERE kd.hapus=0 AND kd.hapus=0 AND kd.id_master_cmt='$idcmt' AND mjp.id_jenis_po='$idjenis' ";
		$query .=" AND mjp.idjenis IN(1,2,3) and mjp.tampil=1  and kd.progress='SETOR' and kd.kategori_cmt='JAHIT' AND pp.tahun=20252026 ";
	...
	public function getStokrincianposetor($idcmt,$idjenis){
		$rp=null;
		$hasil=null;
		$query="SELECT pp.kode_po FROM kelolapo_kirim_setor kd LEFT JOIN produksi_po pp ON(kd.idpo=pp.id_produksi_po) LEFT JOIN master_jenis_po mjp ON(mjp.nama_jenis_po=pp.nama_po) WHERE kd.hapus=0 AND kd.hapus=0 AND kd.id_master_cmt='$idcmt' AND mjp.id_jenis_po='$idjenis' ";
		$query .=" AND mjp.idjenis IN(1,2,3) and mjp.tampil=1  and kd.progress='SETOR' AND pp.tahun=20252026 and kd.kategori_cmt='JAHIT' ";
	...
	public function getStokPO($idcmt,$idjenis){
		$query="SELECT count(*) as jmlpo,SUM(kd.jumlah_pcs-kd.totalsetor) as pcs,mjp.perkalian FROM kirimcmt_detail kd JOIN kirimcmt k ON(k.id=kd.idkirim) LEFT JOIN produksi_po pp ON(kd.kode_po=pp.id_produksi_po) LEFT JOIN master_jenis_po mjp ON(mjp.nama_jenis_po=pp.nama_po) WHERE k.idcmt='$idcmt' AND mjp.id_jenis_po='$idjenis' AND k.hapus=0 AND kd.hapus=0 AND kd.jumlah_pcs<>kd.totalsetor ";
		$dataReturn = $this->db->query($query)->row_array();
		return $dataReturn;
	}
	...
	public function getStokrincianpo($idcmt,$idjenis){
		$rp=null;
		$hasil=null;
		$query="SELECT pp.kode_po FROM kirimcmt_detail kd JOIN kirimcmt k ON(k.id=kd.idkirim) LEFT JOIN produksi_po pp ON(kd.kode_po=pp.id_produksi_po) LEFT JOIN master_jenis_po mjp ON(mjp.nama_jenis_po=pp.nama_po) WHERE k.hapus=0 AND kd.hapus=0 AND k.idcmt='$idcmt' AND mjp.id_jenis_po='$idjenis' AND kd.jumlah_pcs<>kd.totalsetor ";
		$dataReturn = $this->db->query($query)->result_array();
		foreach($dataReturn as $r){
				$rp[]=$r['kode_po'];
